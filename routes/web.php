...
Route::get('/register/start', [RegisteredUserController::class, 'startRegistration'])->name('register.start');

Route::get('/register', function () {
    if (!session()->has('registration.step2.role')) {
        return redirect()->route('register.start');
    }
    return app(App\Http\Controllers\Auth\RegisteredUserController::class)->showRegistrationForm();
})->name('register');
...
Route::prefix('admin')->middleware(['auth'])->group(function () {
    Route::get('/subscriptions', [AdminController::class, 'subscriptionManagement'])
         ->name('admin.subscriptions');
});


Route::prefix('admin')->group(function () {
    Route::get('/active-listings', [AdminController::class, 'activeListings'])
         ->name('admin.active-listings');
});
