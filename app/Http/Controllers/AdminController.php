...
public function subscriptionManagement()
{
    ...
    $subscriptionStats = [
        ...
        'expiring_soon' => Subscription::where('ends_at', '<=', Carbon::now()->addDays(7))
                                      ->where('status', 'active')
                                      ->count(),
    ];
    ...
    {
        $activeListings = Listing::with('seller') // only keep user relationship
                                 ->where('status', 'approved')
        ...
        $listingStats = [
            'total_active' => Listing::where('status', 'approved')
                                     ->where('listing_state', 'active')
                                     ->where('expiry_status', 'active')
                                     ->count(),
            'expired' => Listing::where('listing_state', 'expired')->count(),
            'sold' => Listing::where('status', 'sold')->count(),
            // removed 'with_boosts' since boosts relationship is removed
        ];
