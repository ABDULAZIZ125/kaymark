...
class RegisteredUserController extends Controller
{
    ...
    public function create(): View
    {
        if (!session()->has('registration.step2.role')) {
            // This should ideally be caught by the route, but as a fallback:
    ...
    public function startRegistration(Request $request)
    {
        ...
        if ($role && in_array($role, [User::ROLE_BUYER, User::ROLE_SELLER])) {
            $request->session()->put('registration.step2', ['role' => $role]);
    ...
    public function step2(Request $request)
    {
        ...
        $role = $request->session()->get('registration.step2.role');
        if (!$role) {
            return redirect()->route('register.start')->with('error', 'Please select your role first.');
        }
        ...
        $request->session()->merge('registration.step2', [
